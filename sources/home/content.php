...
if (!empty($pro_users)) {
    ...
    $db->where('privacy', 0);
    $db->orderBy('id', 'DESC');
    $latest_data_music = $db->where('is_movie', 0)->where('user_id', $pt->blocked_array, 'NOT IN')->where('approved', 1)->where('is_short', 0)->where('live_time', 0)->get(T_VIDEOS, $limit);
}

if (empty($latest_data_music)) {
    $db->where('privacy', 0);
    $latest_data_music = $db->where('is_movie', 0)->where('user_id', $pt->blocked_array, 'NOT IN')->where('approved', 1)->where('is_short', 0)->where('live_time', 0)->orderBy('id', 'DESC')->get(T_VIDEOS, $limit);
}
